(function () {
    ...
    // ========================================
    // ОПТИМИЗАЦИИ: Единый Observer и кэширование
    // ========================================
    ...
    function applyLazyLoad() {
        ...
        images.forEach(function(img) {
            if (!img.dataset.lazyRegistered) {
                img.dataset.lazyRegistered = 'true';
    ...
    // Информация о плагине (упрощенная версия)
    function loadPluginInfo(callback) {
        // Заглушка для информации о плагине
        var pluginInfo = {
            name: 'Interface MOD Optimized',
            version: '3.0',
    ...
    function showAboutPlugin() {
        var aboutText = 'Interface MOD v3.0 Optimized\n' +
                       'Оптимизированная версия с:\n' +
                       '• Единым MutationObserver\n' +
                       '• Кэшированием селекторов\n' +
                       '• Lazy Load изображений\n' +
                       '• WebP поддержкой';
        
        Lampa.Noty.show(aboutText);
    }
    ...
    function changeMovieTypeLabels() {
        ...
        function processAllCards() {
            if (!settings.show_movie_type) return;
            getElements('.card').each(function() {
                addLabelToCard(this);
            });
        }
    ...
    function newInfoPanel() {
        ...
        Lampa.Listener.follow('full', function(data) {
            if (data.type === 'complite' && settings.info_panel) {
                setTimeout(function() {
                    var details = getElements('.full-start-new__details');
    ...
    function setupVoteColorsObserver() {
        ...
        var observer = registerObserverCallback(function() { setTimeout(updateVoteColors, 100); });
    }
